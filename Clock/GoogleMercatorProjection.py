...
class MercatorProjection :
    ...
    def fromPointToLatLng(self,point) :
          ...
          lat = radiansToDegrees(2.0 * math.atan(math.exp(latRadians)) - math.pi / 2.0)
...
def getCorners(center, zoom, mapWidth, mapHeight):
    ...
    SWPoint = Point(centerPx.x-(mapWidth/2.0)/scale, centerPx.y+(mapHeight/2.0)/scale)
    SWLatLon = proj.fromPointToLatLng(SWPoint)
    NEPoint = Point(centerPx.x+(mapWidth/2.0)/scale, centerPx.y-(mapHeight/2.0)/scale)
